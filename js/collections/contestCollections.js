...
define (["jquery", "underscore", "backbone",
        "app/views/contestViews", "app/views/contestListViews", "app/views/singleContestViews",
        "app/views/areaStatisticsViews", "app/utils/config", "app/utils/helper"],
    function($, _, Backbone, Views, ContestListViews, SingleContestView, AreaStatistics, Config, Helper){
                ...
                if(_.has(response, 'areaResults')){
                    ...
                    if(!_.has(this, 'areaResults')) {
                        ...
                        if (Timestamp !== response.statistics.timeStamp){
                            this.areaResultsCollections = new Collections.AreaResultsCollection(areaResults);
                        }
        ...
        Collections.StatisticsCollection = Backbone.Collection.extend({
            ...
            onErrorHandler: function (collection, response) {
                ...
                Helper.alertMessage(errorMessage, 'Arearesults (ContestCollection) '+response.status +' ('+response.statusText+')');
            },
            ...
            initialize: function(response){
                ...
                if(Config.showPinnedContest) {
                    ...
                    if(_.has(response, pinnedAreaID)) {
                        var pinnedAreaResults = $.extend(true, {}, response[pinnedAreaID].contestResults);
                        var pinnedContestModel = _.findWhere(pinnedAreaResults, {'id': Config.pinnedContestID});
                        if(pinnedContestModel === undefined){pinnedContestErrors = "Pinned ContestID Error";}
                        Models.pinnedContestModel = new Backbone.Model(pinnedContestModel);
                        Models.pinnedContestModel.choiceResultsCollection = new Collections.ChoiceResultCollection(Models.pinnedContestModel.attributes.choiceResults);
                        pinnedResultsCollections = new Collections.PinnedContestResultCollection(Models.pinnedContestModel);
                ...
                if(_.has(response, selectedAreaID)){
                    ...
                    if(!_.has(this, selectedAreaID)) {
                        ...
                        if(Config.showEligibleVotersTitle){
                            ...
                            $('#eligibleVotersNumber').empty().append(selectedAreaResults.statistics.eligibleVoters);
                            Models.LocalStatsModel = new Backbone.Model(selectedAreaResults.statistics);
                        }
                        this.selectedAreaResultsCollections = new Collections.SelectedAreaResultsCollections(selectedAreaResults);
                    } else {
            ...
            initialize: function(response){
                ...
                if(_.has(response, 'contestResults')){
                    ...
                    if(!_.has(this, 'contestResults')) {
                        this.contestResultCollections = new Collections.ContestResultCollection(contestResults);
            ...
            addView: function(){
                if (this.length > 0) {
                    ...
                    if (Config.isContestListVisible) {
                        ContestListViews.contestView = new ContestListViews.ContestListCollectionView({ collection: this, updated: updated});
                        ContestListViews.contestView.render();
                    } else {
                        Views.contestView = new Views.ContestResultCollectionView({ collection: this, updated: updated });
                ...
                if(this.length > 0){
                    ...
                    //if(updated) {
                        ...
                        if (Helper.checkUpValue(Models.pinnedContestModel.get('id'))) {
                            var pinnedContestView = new SingleContestView.PinnedContestResultCollectionView({ model: Models.pinnedContestModel, updated: updated});
