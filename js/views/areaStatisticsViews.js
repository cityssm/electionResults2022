/* global define */
define(["jquery", "backbone", "underscore", "app/utils/helper", "app/utils/config"], function (
    $,
    Backbone,
    _,
    Helper,
    ...
    $(window).resize(
        _.debounce(function () {
            ...
            if (screenWidth >= Helper.MobileBreakpoint) {
                if ($areaReport.hasClass("moveReportDown")) {
                    $areaReport.removeClass("moveReportDown");
                }
                if ($areaReport.hasClass("collapse")) {
                    $areaReport.removeClass("collapse");
                }
                if ($areaReport.hasClass("in")) {
                    $areaReport.removeClass("in");
                }
                if ($backButton.css("display") === "block") {
                    $backButton.fadeOut("fast");
                }
                if ($areaListGroup.css("display") === "none") {
                    $areaListGroup.fadeIn("fast");
                }
            } else {
                if (!$areaReport.hasClass("collapse")) {
                    $areaReport.addClass("collapse");
                }
                if (!$areaReport.hasClass("moveReportDown")) {
                    $areaReport.addClass("moveReportDown");
                }
                if ($backButton.css("display") === "none" && !Helper.isLoopActive) {
                    $backButton.fadeIn("fast");
                }
                if ($areaListGroup.css("display") === "block") {
    ...
    Views.AreaReportView = Backbone.View.extend({
        ...
        render: function () {
            ...
            this.$el
                ...
                .hide()
                .append(
                    Helper.template("areaReportTemplate" + statsTypeTemplate, this.model.toJSON())
                );
                ...
                $.each(allItems, function (index, element) {
                    ...
                    if ($obj) {
                        if (element === "tabulators" || element === "polls") {
                            var closed =
                                "closed" + (element.charAt(0).toUpperCase() + element.slice(1));
