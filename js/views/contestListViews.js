/* global define */
define (["jquery", "backbone", "app/views/singleContestViews", "app/views/contestViews",
    "text!app/templates/contest.html", "app/utils/helper", "app/utils/config"],
    ...
    Views.ContestListCollectionView = Backbone.View.extend({
        ...
        render: function(){
            ...
            if(Config.isAllContestMenuItemVisible && Views.ContestResultCollection.length>1) {
                ...
                var contestListItemView = new Views.ContestListItemView({model: allContestModel, updated: Updated});
                this.$el.append(contestListItemView.renderAllContestItem().el);
            }

            this.collection.each(function(model){
                contestListItemView = new Views.ContestListItemView({model:model, updated:Updated});
            ...
            if (!Helper.checkUpValue(window.contestID)) {
                ...
                if ($target.length > 0) {
                    ...
                    if(!Helper.isLoopActive) { Helper.alertMessage('errorInvalid', 'Invalid ID'); }
    ...
    Views.ContestListItemView = Backbone.View.extend({
        ...
        onContestButtonClick: function(evt){
            ...
            if(selectedContest !== target) {
                ...
                if(!Updated){window.appRouter.navigate(this.$el.find('a').attr('href'));}
                if(this.model.get('id') === "000"){
                    contestView = new ContestView.ContestResultCollectionView({ collection: Views.ContestResultCollection, updated:Updated});
                }else{
                    contestView = new SingleContestView.ContestResultCollectionView({ model: this.model, updated:Updated});
                }
        ...
        addAttributes: function(){
          ...
          Views.selectedContestID = this.model.get('id');
            ...
            this.$el
                ...
                .find('a').attr('href', '#'+fragment + "ct/"+Views.selectedContestID);
        },
